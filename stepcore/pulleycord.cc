...
void PulleyCord::getConstraintsInfo(ConstraintsInfo* info, int offset)
{
  if(_body1 && _body2)
  {
    ...
    info->value[offset] = (_length1+_length2 - lA - lB);
    info->derivative[offset] = (-velocity1().dot(nA) - velocity2().dot(nB));
    
    if(_p1)
    {
      info->jacobian.coeffRef(offset, _p1->variablesOffset() + RigidBody::PositionOffset) = (-nA[0]);
      info->jacobian.coeffRef(offset, _p1->variablesOffset() + RigidBody::PositionOffset+1) = (-nA[1]);
      
      info->jacobianDerivative.coeffRef(offset, _p1->variablesOffset() + RigidBody::PositionOffset) = (-velocity1()[0]);
      info->jacobianDerivative.coeffRef(offset, _p1->variablesOffset() + RigidBody::PositionOffset+1) = (-velocity1()[1]); 
    }
    else if(_r1)
    {
      ...
      info->jacobianDerivative.coeffRef(offset, _r1->variablesOffset() + RigidBody::PositionOffset) = (-velocity1()[0]);
      info->jacobianDerivative.coeffRef(offset, _r1->variablesOffset() + RigidBody::PositionOffset+1) = -(velocity1()[1]);
      info->jacobianDerivative.coeffRef(offset, _r1->variablesOffset() + RigidBody::AngleOffset) =  
                                                       (v1[0]*r1[1] - v1[1]*r1[0] + _r1->angularVelocity()*uA.dot(r1));
    }
    ...
    if(_p2)
    {
      ...
      info->jacobianDerivative.coeffRef(offset, _p2->variablesOffset() + Particle::PositionOffset) = (-velocity2()[0]);
      info->jacobianDerivative.coeffRef(offset, _p2->variablesOffset() + Particle::PositionOffset+1) = (-velocity2()[1]); 
    }
    else if(_r2)
    {
      ...
      info->jacobianDerivative.coeffRef(offset, _r2->variablesOffset() + RigidBody::PositionOffset) = (-velocity2()[0]);
      info->jacobianDerivative.coeffRef(offset, _r2->variablesOffset() + RigidBody::PositionOffset+1) = (-velocity2()[1]);
      info->jacobianDerivative.coeffRef(offset, _r2->variablesOffset() + RigidBody::AngleOffset) = 
                                                              (v2[0]*r2[1] - v2[1]*r2[0] + _r2->angularVelocity()*uB.dot(r2));
