...
int GenericEulerSolver::doReverseStep(double t, double stepSize, VectorXd* y, VectorXd* yvar)
{
  _localError = 0;
  _localErrorRatio = 0;
...
int GenericEulerSolver::doReverseEvolve(double* t, double t1, VectorXd* y, VectorXd* yvar)
{
  // XXX: add better checks
  // XXX: replace asserts by error codes here
  //      or by check in world
  ...
  while(*t > t1) {
    STEPCORE_ASSERT_NOABORT(*t-t1 > realStepSize/1000);
